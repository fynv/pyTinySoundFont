...
void Synth(const float* input, float* outputBuffer, unsigned numSamples, NoteState& noteState, const SynthCtrl_Header& controlHeader, const SynthCtrlPnt* controlPnts, unsigned num_ctrl_pnts)
{
	...
	float* outR = (controlHeader.outputmode == STEREO_UNWEAVED ? outL + numSamples : nullptr);

	unsigned tmpLoopStart = controlHeader.loopStart;
	unsigned tmpLoopEnd = controlHeader.loopEnd;
	unsigned tmpEnd = controlHeader.end;
	...
	while (numSamples)
	{
		...
		int blockSamples = (numSamples > controlHeader.effect_sample_block ? controlHeader.effect_sample_block : numSamples);
		numSamples -= blockSamples;

		if (i_ctrl<num_ctrl_pnts)
			ctrlPnt = controlPnts[i_ctrl];
		else
			break;
		...
		gainLeft = gainMono *controlHeader.panFactorLeft;
		gainRight = gainMono  * controlHeader.panFactorRight;
		...
		while (blockSamples-- && tmpSourceSamplePosition < tmpSampleEndDbl)
		{
			...
			switch (controlHeader.outputmode)
			{
